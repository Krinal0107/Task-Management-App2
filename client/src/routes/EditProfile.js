...
const EditProfile = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API}/api/user/me`, {
            headers: {
                Authorization: token
            }
        })
            ...
            .catch(error => {
                if (error.response.data.error === "User is not authorized, try logging in again" || error.response.data.error === "User is not authorized and no token, try logging in") {
                    ...
                    toast.error("An error occurred while getting user detail.");
    ...
    const handleEditProfile = async e => {
        ...
        await axios.put(`${process.env.REACT_APP_API}/api/user/me/edit/${id}`,
            { username, email },
            {
            headers: {
                Authorization: token,
