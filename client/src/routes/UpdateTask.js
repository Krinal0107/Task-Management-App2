...
const UpdateTask = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API}/api/tasks/${id}`, {
            headers: {
                Authorization: token,
            }
        })
            ...
            .catch(error => {
                if (error.response.data.error === "User is not authorized, try logging in again" || error.response.data.error === "User is not authorized and no token, try logging in") {
                    ...
                    toast.err("An error occurred while getting the task detail.")
    ...
    const handleUpdateTask = async (e, id) => {
        e.preventDefault();
        try {
            const res = await axios.put(`${process.env.REACT_APP_API}/api/tasks/update/${id}`,
